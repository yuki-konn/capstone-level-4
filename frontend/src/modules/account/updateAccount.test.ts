...
describe("updateAccount function", () => {
  ...
  it("returns error message when the email (partition key) is in the list but new attribute values are not in the list.", async () => {
    // ARRANGE
    const account: any = {
      email: "test3@email.com",
      forSale: true,
    ...
    // ASSERT
    expect(result).toBe(
      "Your account wasn't able to update because the changes made are invalid."
    );
  });
